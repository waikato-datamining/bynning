...
class MinBinSize(BinningPostProcessor[KeyType, int, ItemType]):
    """
    ...
    def post_process_binning(self, binning: Binning[KeyType, int, ItemType]) -> Binning[KeyType, int, ItemType]:
        ...
        new_bins: List[Bin[int, KeyType]] = [Bin(0)]

        # Process each bin in turn
        for bin in binning:
            # Get the next bin to fill up
            new_bin: Bin[int, KeyType] = new_bins[-1]
        ...
        new_binning: Binning[KeyType, int, ItemType] = Binning(binning._binner)
